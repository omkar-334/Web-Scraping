...
def sync_predict(url) -> tuple:
    try:
        with sync_playwright() as p:
            ...
            # pfp = page.get_by_text('What are Public Figure Predictions? How are they linked to Metaculus forecasts?')
            # if pfp:
            #     return None, None, None, "PFP", None, None, None
...
def scrape(status: str, category: str = None, df=None):
    ...
    (
        ...
        df["pred1"],
        df["pred2"],
    ) = zip(*df["page_url"].apply(sync_predict))
...
def filter(df):
    ...
    filter = df[~df["page_url"].str.contains("notebook|prediction")]

    empty = filter[filter["pred1"].isna()]
    empty = scrape("open", df=empty)

    filter = filter.dropna(subset=["pred1"])
    filter = pd.concat([filter, empty])
    return filter
